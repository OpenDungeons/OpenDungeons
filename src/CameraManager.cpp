...
/*! \brief tells the camera to move or rotate in a specific direction
 *
 */
void CameraManager::move(Direction direction)
{
    switch(direction)
    {
        ...
        case moveUp:
            zChange += moveSpeed; // Move camera straight up
            break;

        case moveDown:
            zChange -= moveSpeed; // Move camera straight down
            break;

        case rotateLeft:
            swivelDegrees += 1.3 * rotateSpeed; // Rotate camera left
            break;

        case rotateRight:
            swivelDegrees -= 1.3 * rotateSpeed; // Rotate camera right
            break;

        case rotateUp:
            mRotateLocalVector.x += rotateSpeed.valueDegrees(); // Rotate camera up
            break;

        case rotateDown:
            mRotateLocalVector.x -= rotateSpeed.valueDegrees(); // Rotate camera down
            break;
