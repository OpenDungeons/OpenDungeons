        ...
        switch (curReq->type)
        {
            case RenderRequest::refreshTile:
                ...
                if (mSceneMgr->hasSceneNode(curTile->name + "_node"))
                {
                    ...
                    mSceneMgr->getSceneNode(curTile->name + "_node")->detachObject(
                            curTile->name);
                    mSceneMgr->destroyEntity(curTile->name);

                    // Create the new mesh
                    string tileTypeString = Tile::tileTypeToString(
                            curTile->getType());
                    snprintf(meshName, sizeof(meshName), "%s%i.mesh",
                            tileTypeString.c_str(),
            ...
            case RenderRequest::createRoom:
                ...
                node = roomSceneNode->createChildSceneNode(tempSS.str()
                        + "_node");
            ...
            case RenderRequest::createRoomObject:
                ...
                tempString = curRoomObject->getOgreNamePrefix()
                        + curRoomObject->getName();
                ent = mSceneMgr->createEntity(tempString,
            ...
            case RenderRequest::createCreature:
                ...
                ent = mSceneMgr->createEntity("Creature_" + curCreature->name,
                        curCreature->meshName);
                colourizeEntity(ent, curCreature->color);
                node = creatureSceneNode->createChildSceneNode(
            ...
            case RenderRequest::orientSceneNodeToward:
                ...
                tempVector = node->getOrientation()
            ...
            case RenderRequest::createWeapon:
                ...
                ent = mSceneMgr->getEntity("Creature_" + curCreature->name);
                colourizeEntity(ent, curCreature->color);
                weaponEntity = mSceneMgr->createEntity("Weapon_"
                ...
                tempQuaternion.FromAngleAxis(Degree(-90.0), Ogre::Vector3(1.0,
                        0.0, 0.0));
            ...
            case RenderRequest::destroyWeapon:
                ...
                {
                    ent = mSceneMgr->getEntity("Weapon_"
                            + curWeapon->handString + "_" + curCreature->name);
                    mSceneMgr->destroyEntity(ent);
                }
                break;
            ...
            case RenderRequest::createMissileObject:
                ...
                ent = mSceneMgr->createEntity(curMissileObject->name,
            ...
            case RenderRequest::destroyMissileObject:
                ...
                if (mSceneMgr->hasEntity(curMissileObject->name))
                {
                    ...
                    node = mSceneMgr->getSceneNode(curMissileObject->name
                            + "_node");
                    node->detachObject(ent);
                    creatureSceneNode->removeChild(node);
                    mSceneMgr->destroyEntity(ent);
                    mSceneMgr->destroySceneNode(curMissileObject->name
                            + "_node");
                }
                break;
            ...
            case RenderRequest::createMapLight:
                ...
                node = lightSceneNode->createChildSceneNode(tempString
                        + "_node");
                ...
                if (serverSocket == NULL && clientSocket == NULL)
                {
                    ...
                    tempString2 = (string) "MapLightIndicator_"
                            + curMapLight->getName();
                ...
                node2
                        = node->createChildSceneNode(tempString
                                + "_flicker_node");
            ...
            case RenderRequest::destroyMapLight:
                ...
                if (mSceneMgr->hasLight(tempString))
                {
                    ...
                    node = mSceneMgr->getSceneNode(tempString + "_node");
                    node2 = mSceneMgr->getSceneNode(tempString
                            + "_flicker_node");
            ...
            case RenderRequest::destroyMapLightVisualIndicator:
                ...
                {
                    node = mSceneMgr->getSceneNode(tempString + "_node");
                    tempString = (string) "MapLightIndicator_"
                            + curMapLight->getName();
                    if (mSceneMgr->hasEntity(tempString))
                    {
                        ent = mSceneMgr->getEntity(tempString);
            ...
            case RenderRequest::createField:
                ...
                while (fieldItr != curField->end())
                {
                    ...
                    node = fieldSceneNode->createChildSceneNode(tempSS.str()
                            + "_node");
            ...
            case RenderRequest::refreshField:
                ...
                while (fieldItr != curField->end())
                {
                    ...
                    else
                    {
                        ...
                        ent = mSceneMgr->createEntity(tempSS.str(),
                                "Field_indicator.mesh");
                        node = fieldSceneNode->createChildSceneNode(
                                tempSS.str() + "_node");
                        node->attachObject(ent);
                    }
            ...
            case RenderRequest::createCreatureVisualDebug:
                ...
                if (curTile != NULL && curCreature != NULL)
                {
                    ...
                    node = creatureSceneNode->createChildSceneNode(tempSS.str()
                            + "_node");
                    node->attachObject(ent);
                    node->setPosition(Ogre::Vector3(curTile->x, curTile->y, 0));
                    node->setScale(Ogre::Vector3(BLENDER_UNITS_PER_OGRE_UNIT,
            ...
            case RenderRequest::setObjectAnimationState:
                ...
                if (ent->hasSkeleton() && ent->getSkeleton()->hasAnimation(
                        curReq->str))
                {
                    ...
                    curAnimatedObject->animationState = ent->getAnimationState(
                            curReq->str);
