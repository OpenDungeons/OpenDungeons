#-------------------------------------------------------------------
# CMake script to locate OgreOggSound
# Based on Ogre cmake find scripts http://www.ogre3d.org/
#
...
# OS specific guesses
if (WIN32)
  set(OgreOggSound_PREFIX_GUESSES
    ${ENV_PROGRAMFILES}/OgreOggSound
    C:/OgreOggSound
  )
elseif (UNIX)
  set(OgreOggSound_PREFIX_GUESSES
    /opt/ogreoggsound
    /opt/OGREOGGSOUND
    /usr/lib${LIB_SUFFIX}/ogreoggsound
    /usr/lib${LIB_SUFFIX}/OGREOGGSOUND
    /usr/local/lib${LIB_SUFFIX}/ogreoggsound
    /usr/local/lib${LIB_SUFFIX}/OGREOGGSOUND
	/usr/lib${LIB_SUFFIX}
	/usr/local/lib${LIB_SUFFIX}
    $ENV{HOME}/ogreoggsound
    $ENV{HOME}/OGREOGGSOUND
	$ENV{HOME}/OgreOggSound
  )
endif ()
set(OgreOggSound_PREFIX_PATH
  ${OgreOggSound_PREFIX_GUESSES}
)
create_search_paths(OgreOggSound)
# If both OGRE_BUILD and OGRE_SOURCE are set, prepare to find Ogre in a build dir
# set(OgreOggSound_PREFIX_SOURCE ${OGRE_SOURCE} ${ENV_OGRE_SOURCE})
# set(OgreOggSound_PREFIX_BUILD ${OGRE_BUILD} ${ENV_OGRE_BUILD})
#set(OgreOggSound_PREFIX_DEPENDENCIES_DIR ${OGRE_DEPENDENCIES_DIR} ${ENV_OGRE_DEPENDENCIES_DIR})
if (OgreOggSound_PREFIX_SOURCE AND OgreOggSound_PREFIX_BUILD)
  foreach(dir ${OgreOggSound_PREFIX_SOURCE})
    set(OgreOggSound_INC_SEARCH_PATH ${dir}/include ${OgreOggSound_INC_SEARCH_PATH})
    set(OgreOggSound_LIB_SEARCH_PATH ${dir}/lib ${OgreOggSound_LIB_SEARCH_PATH})
    set(OgreOggSound_BIN_SEARCH_PATH ${dir}/lib ${OgreOggSound_BIN_SEARCH_PATH})
  endforeach(dir)
  foreach(dir ${OgreOggSound_PREFIX_BUILD})
    set(OgreOggSound_INC_SEARCH_PATH ${dir}/include ${OgreOggSound_INC_SEARCH_PATH})
    set(OgreOggSound_LIB_SEARCH_PATH ${dir}/lib ${OgreOggSound_LIB_SEARCH_PATH})
    set(OgreOggSound_BIN_SEARCH_PATH ${dir}/lib ${OgreOggSound_BIN_SEARCH_PATH})
  endforeach(dir)
  ...
  set(OgreOggSound_PREFIX_SOURCE "NOTFOUND")
  set(OgreOggSound_PREFIX_BUILD "NOTFOUND")
endif ()
...
use_pkgconfig(OgreOggSound_PKGC "OgreOggSound${OgreOggSound_LIB_SUFFIX}")

if(NOT OgreOggSound_BUILD_PLATFORM_IPHONE)
  # try to find framework on OSX
  findpkg_framework(OgreOggSound)
else()
	set(OgreOggSound_LIBRARY_FWK "")
endif()
...
if (OgreOggSound_INCLUDE_DIR)
  ...
  pkg_message(OGRE "Found OgreOggSound")# ${OGRE_VERSION_NAME} (${OGRE_VERSION})")
...
find_library(OgreOggSound_LIBRARY_REL NAMES ${OgreOggSound_LIBRARY_NAMES} HINTS ${OgreOggSound_LIB_SEARCH_PATH} ${OgreOggSound_PKGC_LIBRARY_DIRS} ${OgreOggSound_FRAMEWORK_SEARCH_PATH} PATH_SUFFIXES "" "release" "relwithdebinfo" "minsizerel")
find_library(OgreOggSound_LIBRARY_DBG NAMES ${OgreOggSound_LIBRARY_NAMES_DBG} HINTS ${OgreOggSound_LIB_SEARCH_PATH} ${OgreOggSound_PKGC_LIBRARY_DIRS} ${OgreOggSound_FRAMEWORK_SEARCH_PATH} PATH_SUFFIXES "" "debug")
make_library_set(OgreOggSound_LIBRARY)

if (OgreOggSound_INCLUDE_DIR AND OgreOggSound_LIBRARY)
	message("-- OgreOggSound found")
endif ()

if(APPLE)
  set(OgreOggSound_LIBRARY_DBG ${OgreOggSound_LIB_SEARCH_PATH})
endif()
# if (OGRE_INCOMPATIBLE)
  # set(OGRE_LIBRARY "NOTFOUND")
# endif ()

set(OgreOggSound_INCLUDE_DIR ${OgreOggSound_INCLUDE_DIR})
list(REMOVE_DUPLICATES OgreOggSound_INCLUDE_DIR)
...
get_filename_component(OgreOggSound_FOUND_LIBRARY_DIR_REL "${OgreOggSound_FOUND_LIBRARY_REL}" PATH)
get_filename_component(OgreOggSound_FOUND_LIBRARY_DIR_DBG "${OgreOggSound_FOUND_LIBRARY_DBG}" PATH)
set(OgreOggSound_FOUND_LIBRARY_DIRS ${OgreOggSound_FOUND_LIBRARY_DIR_REL} ${OgreOggSound_FOUND_LIBRARY_DIR_DBG})

# find binaries
if (NOT OgreOggSound_FOUND_STATIC)
	if (WIN32)
