...
bool AIWrapper::shouldGroundTileBeConsideredForBestPlaceForRoom(Tile* tile, Seat* playerSeat)
{
    switch(tile->getType())
    {
        ...
        case Tile::TileType::claimed:
        {
            ...
            if(!tile->isClaimedForSeat(playerSeat))
                return false;
            ...
            {
                if(t->isClaimedForSeat(playerSeat) &&
...
bool AIWrapper::shouldWallTileBeConsideredForBestPlaceForRoom(Tile* tile, Seat* playerSeat)
{
    ...
    if(tile->isWallClaimedForSeat(playerSeat))
        return true;
...
bool AIWrapper::findBestPlaceForRoom(Tile* tile, Seat* playerSeat, int32_t wantedSize, bool useWalls,
    int32_t& bestX, int32_t& bestY)
{
    ...
    int32_t maxOffset = std::max(gameMap.getMapSizeX(), gameMap.getMapSizeY());
    ...
    for(int32_t offset = 1; offset < maxOffset; ++offset)
    {
        ...
        for(int32_t k = 0; k < nbTiles; ++k)
        {
            Tile* t;
            // North
            t  = gameMap.getTile(tile->getX() - offset - wantedSize + 2 + k, tile->getY() + offset);
            if((t != nullptr) &&
               computePointsForRoom(t, playerSeat, wantedSize, true, useWalls, points))
            {
                points -= handicap;
            ...
            // East
            t  = gameMap.getTile(tile->getX() + offset, tile->getY() - k + offset);
            if((t != nullptr) &&
               computePointsForRoom(t, playerSeat, wantedSize, true, useWalls, points))
            {
                points -= handicap;
            ...
            t  = gameMap.getTile(tile->getX() + offset + wantedSize - 2 - k, tile->getY() - offset);
            if((t != nullptr) &&
               computePointsForRoom(t, playerSeat, wantedSize, false, useWalls, points))
            {
                points -= handicap;
                int32_t centerX = t->getX() - (wantedSize / 2);
                int32_t centerY = t->getY() - (wantedSize / 2);
            ...
            // West
            t  = gameMap.getTile(tile->getX() - offset, tile->getY() - offset + k);
            if((t != nullptr) &&
               computePointsForRoom(t, playerSeat, wantedSize, false, useWalls, points))
            {
                points -= handicap;
...
bool AIWrapper::computePointsForRoom(Tile* tile, Seat* playerSeat, int32_t wantedSize,
    bool bottomLeft2TopRight, bool useWalls, int32_t& points)
{
    ...
    for(int32_t xx = 0; xx < wantedSize; ++xx)
    {
        for(int32_t yy = 0; yy < wantedSize; ++yy)
        {
            ...
            if(bottomLeft2TopRight)
                t  = gameMap.getTile(tileX + xx, tileY + yy);
            else
                t  = gameMap.getTile(tileX - xx, tileY - yy);
            ...
            if(!shouldGroundTileBeConsideredForBestPlaceForRoom(t, playerSeat))
            {
                isOk = false;
                break;
            }
    ...
    for(int32_t kk = 0; kk < wantedSize; ++kk)
    {
        Tile* t;
        if(bottomLeft2TopRight)
            t  = gameMap.getTile(tileX - 1, tileY + kk);
        else
            t  = gameMap.getTile(tileX + 1, tileY - kk);
        if(t == nullptr)
            continue;

        if(shouldWallTileBeConsideredForBestPlaceForRoom(t, playerSeat))
            ++nbConsecutiveTiles;
        else
            nbConsecutiveTiles = 0;
    ...
    for(int32_t kk = 0; kk < wantedSize; ++kk)
    {
        Tile* t;
        if(bottomLeft2TopRight)
            t = gameMap.getTile(tileX + wantedSize, tileY + kk);
        else
            t = gameMap.getTile(tileX - wantedSize, tileY - kk);
        if(t == nullptr)
            continue;
    ...
    for(int32_t kk = 0; kk < wantedSize; ++kk)
    {
        Tile* t;
        if(bottomLeft2TopRight)
            t = gameMap.getTile(tileX + kk, tileY - 1);
        else
            t = gameMap.getTile(tileX - kk, tileY + 1);
        if(t == nullptr)
            continue;
    ...
    for(int32_t kk = 0; kk < wantedSize; ++kk)
    {
        Tile* t;
        if(bottomLeft2TopRight)
            t = gameMap.getTile(tileX + kk, tileY + wantedSize);
        else
            t = gameMap.getTile(tileX - kk, tileY - wantedSize);
        if(t == nullptr)
            continue;
...
bool AIWrapper::digWayToTile(Tile* tileStart, Tile* tileEnd)
{
    ...
    std::list<Tile*> pathToDig = gameMap.path(tileEnd, tileStart, kobold,
        seat, true);
    if (pathToDig.empty())
        return false;
    ...
    for(std::list<Tile*>::iterator it = pathToDig.begin(); it != pathToDig.end();)
    {
        ...
        if(!isPathFound &&
           ...
           (gameMap.pathExists(kobold, tileStart, tile)))
        {
            isPathFound = true;
        }
        ...
        for(Tile* t : tile->getAllNeighbors())
        {
            if(!isPathFound &&
               (t->getFullness() == 0.0) &&
               (gameMap.pathExists(kobold, tileStart, t)))
            {
